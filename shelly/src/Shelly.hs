...
module Shelly
       (
         ...
         , followSymlink
         -- * accessor functions
         , setCreateProcessGroup
         , setCallback
         ) where

import Shelly.Base
import Shelly.Find
import Control.Monad ( filterM, forM, liftM2, unless, void, when )
import Control.Monad.Trans ( MonadIO )
import Control.Monad.Reader (ask)
#if defined(__GLASGOW_HASKELL__) && __GLASGOW_HASKELL__ < 706
import Prelude hiding ( FilePath, catch, readFile )
#else
...
import System.IO.Error ( catchIOError, isEOFError, isIllegalOperation, isPermissionError )
import System.Exit
...
import System.Process ( CmdSpec(..), CreateProcess(..), ProcessHandle, StdStream(CreatePipe, UseHandle), StdStream(..)
...
import System.Directory ( Permissions(..), getPermissions, getTemporaryDirectory, setPermissions )
...
shellyProcess reusedHandles st cmdSpec =  do
    (createdInH, createdOutH, createdErrorH, pHandle) <- createProcess CreateProcess {
        ...
        , create_group = sCreateGroup st -- See setCreateProcessGroup. Setting this to True is essential for OS job control.
#endif
  ...
  where
    ...
    toHandle (Just NoStream)      = error "error in shelly creating process: NoStream"
...
runHandles exe args reusedHandles withHandles = do
    ...
    bracket_sh
      ...
      (\(inH,outH,errH,procH) -> do
        case sCallback state of
          Just callback -> liftIO $ callback procH
          _ -> return ()
...
-- | setCreateProcessGroup True = create new process group on create process
setCreateProcessGroup :: Bool -> Sh ()
setCreateProcessGroup p = modify $ \state -> state { sCreateGroup = p }

-- | setCallback (Just callback) = run callback on create process
setCallback :: Maybe (ProcessHandle -> IO ()) -> Sh ()
setCallback callback = modify $ \state -> state { sCallback = callback }                    
